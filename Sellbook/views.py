...
def home(request):
    ...
    # If a category ID is provided in the URL, filter books based on that category
    if category_id:
        try:
    ...
    # If a search query is provided, filter books based on the title containing the search query
    if search_query:
...
def seller_profile(request):
    ...
    if request.user.is_authenticated and hasattr(request.user, 'seller_profile'):
        ...
        seller_books = seller_profile.book_set.all()  # Assuming your Book model has a ForeignKey to Seller_Profile
        context = {
            'seller_profile': seller_profile,
            'seller_books': seller_books  # Pass the seller's books to the template context
        }
